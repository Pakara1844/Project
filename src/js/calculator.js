...
// Max allowed spread (µF) of the per-phase SERIES capacitance WITHIN one wye side
// (A vs B vs C on Y1, A' vs B' vs C' on Y2). The BALANCE GOAL is A=B=C on Y1 AND
// A'=B'=C' on Y2 — each wye internally balanced (the two wyes need NOT match each
// other; that is not required for I_no=0). `phaseSpreadOf().max` = the worse of the
// two within-wye spreads and must be ≤ this tolerance. An ADDITIONAL acceptance goal
// beyond the I_no alarm; display + swap-priority only — it NEVER changes the I_no
// calculation. (YY topology only.)
// SIZING NOTE (measured, 115 kV / 10 caps-in-series per phase → C_phase ≈ 2.72 µF):
// ΔC within a wye maps to I_no at roughly 0.001 µF ≈ 10.5 mA. So 0.1 µF
// would allow I_no ≈ 1050 mA — useless as a gate. 0.005 µF ≈ 52 mA, which lines up
// with the default 50 mA alarm. Keep this in the same order as the alarm you expect;
// a looser value makes the balance badge pass while the relay still trips.
...
// capacitances (perPhase.X.C1 = Y1, .C2 = Y2). The BALANCE GOAL is WITHIN each wye —
// A=B=C on Y1 AND A'=B'=C' on Y2 — NOT that all six phases match across wyes. So
...
// is kept for display only and does NOT gate anything.
function phaseSpreadOf(metrics, tol) {
  ...
  const s1  = sp([p.A.C1, p.B.C1, p.C.C1]);                              // within Y1: A=B=C
  const s2  = sp([p.A.C2, p.B.C2, p.C.C2]);                              // within Y2: A'=B'=C'
  const all = sp([p.A.C1, p.B.C1, p.C.C1, p.A.C2, p.B.C2, p.C.C2]);      // all six (display only)
...
function calcYYMetrics(y1, y2, systemKV) {
  ...
  // Per-phase detail (phasor contribution magnitude, for display only)
  const perPhase = {
...
function findPhaseAwareYYSwaps(y1Swap, y2Swap, spare, lockedBaseline, systemKV, lockedY1Arr, lockedY2Arr, nameplate, swapMode, thresholdMA) {
  ...
  // Relay threshold (mA). PRIMARY objective = minimize the WITHIN-wye series-C spread
  // (A=B=C on Y1, A'=B'=C' on Y2), but a relay-PASSING arrangement (I_no < THR) always
  // beats a failing one — "balance เป็นหลัก + I_no ต้องผ่านรีเลย์ด้วย". Infinity = no gate.
  ...
  // WITHIN-wye series-C spread (the primary balance metric = max of the Y1 A/B/C
  // spread and the Y2 A'/B'/C' spread). PRIMARY objective = minimize that within-wye
  // spread (balance A=B=C on Y1 and A'=B'=C' on Y2); I_no is the relay constraint.
  // Same cost as computing I_no alone — spread is derived from the same result.
