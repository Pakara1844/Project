...
// (phase A vs B vs C). After swapping, each side should have its three phases'
// series-combined C within this tolerance — an ADDITIONAL acceptance goal beyond
// the I_no alarm. Like nameplate, this is display + swap-priority only; it NEVER
// changes the I_no calculation. (YY topology only.)
// SIZING NOTE (measured, 115 kV / 10 caps-in-series per phase → C_phase ≈ 2.72 µF):
// ΔC across the six phases maps to I_no at roughly 0.001 µF ≈ 10.5 mA. So 0.1 µF
...
// capacitances (perPhase.X.C1 = Y1, .C2 = Y2). `.all` = spread across ALL SIX
// phases together (A,B,C,A',B',C') — this is the primary phase-balance metric the
// optimizer minimizes (star-point balance goal ≤ PHASE_SPREAD_UF). `.y1`/`.y2` are
// the per-side spreads, kept for display detail. `.max` aliases `.all`.
function phaseSpreadOf(metrics, tol) {
  ...
  const s1  = sp([p.A.C1, p.B.C1, p.C.C1]);                              // within Y1 (N1)
  const s2  = sp([p.A.C2, p.B.C2, p.C.C2]);                              // within Y2 (N2)
  const all = sp([p.A.C1, p.B.C1, p.C.C1, p.A.C2, p.B.C2, p.C.C2]);      // all six phases
  return { y1: s1, y2: s2, all: all, max: all, ok: all <= t + 1e-9, tol: t };
}
...
function findPhaseAwareYYSwaps(y1Swap, y2Swap, spare, lockedBaseline, systemKV, lockedY1Arr, lockedY2Arr, nameplate, swapMode, thresholdMA) {
  ...
  // 6-phase series-C spread (the primary balance metric). PRIMARY objective =
  // minimize spread (balance all phases at the star points); I_no is the relay
  // constraint. Same cost as computing I_no alone — spread is derived from the
  // same result.
  function metricsOf(cY1, cY2) {
    ...
    return { ino: m.Ino_mA, spread: phaseSpreadOf(m).all };
  }
